...
##########---------------------------Logscale bitscores TPs_FPs-plots--------------

#R CMD BATCH Fig3_LogscaleROCplots_Bitscores_max50_1vs1.R
...
pdf(paste(Output_dir,"Logscale_bitscores_max50_1vs1_pfam.pdf", sep = ""))

for (j in 1) {
	for(i in 1:length(meth1)){
   		oname = paste(Input_dir, meth1[i], sep="")
   		cumdata_log <- assign(oname, read.table(paste(oname, datb[j], sep="")))
		d.f <- data.frame( x = cumdata_log[,1], y = cumdata_log[,2])
		if ( j == 1){
			if (meth1[i]=="csblast"){
				plot(x~y, data = d.f, log = "x", type="l", lty=lntp11[i], lwd=lntp2[i], col=plot_colors1[i], xlab= "False positives", ylab= "True positives", main = datb2[j],ylim = c(3144,5244))#,xlim = c(1,1000))

				legend('bottomright', lnames_pf,title = "Search tools: AUC1000 scores", lty = lntp11,col = plot_colors1,cex=1.1,inset= 0.00,lwd = 3)
       			}else {
				lines(x~y,data = d.f,log = "x",type="l",lty=lntp11[i],lwd=lntp2[i], col=plot_colors1[i])
			}
			}
}
}
...
pdf(paste(Output_dir,"Logscale_bitscores_max50_1vs1_supfam.pdf", sep = ""))
for (j in 2) {
	for(i in 1:length(meth2)){
		...
		if ( j == 2){
			if (meth2[i]=="phmmer"){
				plot(x~y,data = d.f,log = "x", type="l",lty=lntp12[i],lwd=lntp2[i], col=plot_colors2[i],xlab= "False positives",ylab= "True positives",main = datb2[j],ylim = c(2346,5046))#,xlim = c(1,1000))	
		
				legend('bottomright', lnames_sf,title = "Search tools: AUC1000 scores", lty = lntp12,col = plot_colors2,cex=1.1,inset= 0.00,lwd = 3)
		   	}else {
				lines(x~y,data = d.f,log = "x", type="l",lty=lntp12[i],lwd=lntp2[i], col=plot_colors2[i])
			}
			}
}
}
...
pdf(paste(Output_dir,"Logscale_bitscores_max50_1vs1_gene3d.pdf", sep = ""))
for (j in 3) {
	for(i in 1:length(meth3)){
		...
		if ( j == 3){
			if (meth3[i]=="csblast"){
				plot(x~y,data = d.f,log = "x", type="l",lty=lntp13[i],lwd=lntp2[i], col=plot_colors3[i],xlab= "False positives",ylab= "True positives",main = datb2[j],ylim = c(2855,5655))#,xlim = c(1,1000))

				legend('bottomright', lnames_gd,title = "Search tools: AUC1000 scores", lty = lntp13,col = plot_colors3,cex=1.1,inset= 0.00,lwd = 3)
      			}else {
				lines(x~y,data = d.f,log = "x", type="l",lty=lntp13[i],lwd=lntp2[i],col=plot_colors3[i])
			}
			}
}
}
